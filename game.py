...
class Game:
    ...
    def __init__(self):
        ...
        self.portal_blue = Portal(self.maze.layout, "blue", CELL_WIDTH, CELL_HEIGHT)
        self.portal_orange = Portal(self.maze.layout, "orange", CELL_WIDTH, CELL_HEIGHT)
    ...
    def handle_portal_teleportation(self):
        ...
        self.portal_blue.x,
        self.portal_blue.y,
        self.portal_blue.width,
        self.portal_blue.height
        )

        orange_rect = pygame.Rect(
        self.portal_orange.x,
        self.portal_orange.y,
        self.portal_orange.width,
        self.portal_orange.height
        )

        if pac_rect.colliderect(blue_rect):
            self.teleport_pacman(self.portal_orange)
            self.portal_cooldown = FPS // 2 

        if pac_rect.colliderect(orange_rect):
            self.teleport_pacman(self.portal_blue)
            self.portal_cooldown = FPS // 2

        for ghost in self.ghosts:
            ghost_rect = ghost.get_rect()

            if ghost_rect.colliderect(blue_rect):
                self.teleport_ghost(ghost, self.portal_orange)
                self.portal_cooldown = FPS // 2
            
            if ghost_rect.colliderect(orange_rect):
                self.teleport_ghost(ghost, self.portal_blue)
                self.portal_cooldown = FPS // 2
    
    def teleport_pacman(self, target_portal):
        self.pacman.x = target_portal.x
        self.pacman.y = target_portal.y

        self.pacman.reset_movement_buffer()
    
    def teleport_ghost(self, ghost, target_portal):
        ghost.x = target_portal.x
        ghost.y = target_portal.y
    ...
    def update(self):
        ...
        if self.game_state == PLAYING:
            ...
            self.handle_portal_teleportation()
    ...
    def draw_game(self):
        ...
        # Draw portals
        self.portal_blue.draw(self.screen)
        self.portal_orange.draw(self.screen)
